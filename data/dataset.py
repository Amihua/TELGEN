...
# 尝试加载 torch_sparse；如果失败，用 torch.nonzero 构造等价的 COO 表示作为兜底
...
def _dense_to_coo(A: torch.Tensor):
    """Fallback for SparseTensor.from_dense(A, has_value=True).storage.{_row,_col,_value}."""
    if SparseTensor is not None:
        sp_a = SparseTensor.from_dense(A, has_value=True)
        return sp_a.storage._row, sp_a.storage._col, sp_a.storage._value
...
class LPDataset(InMemoryDataset):
    ...
    def __init__(
        ...
        # PyTorch 2.6: torch.load 默认 weights_only=True，这里需要完整对象反序列化
    ...
    def process(self):
        # Find the specific ER data file we need to process
        target_file = None
        for f in os.listdir(self.raw_dir):
            if f.endswith('.pkl.gz') and f.startswith(self.extra_path):
                target_file = f
                break

        if target_file is None:
            raise FileNotFoundError(f"Could not find a data file starting with '{self.extra_path}' in {self.raw_dir}")

        data_list = []
        
        print(f"Processing file: {target_file}")
        with gzip.open(os.path.join(self.raw_dir, target_file), "rb") as file:
            ip_pkgs = pickle.load(file)

        for ip_idx in tqdm(range(len(ip_pkgs)), desc=f"Processing instances in {target_file}"):
            if len(ip_pkgs[ip_idx]) == 5:
                # New format: (A_ub, b_ub, A_eq, b_eq, c)
                (A_ub, b_ub, A_eq, b_eq, c) = ip_pkgs[ip_idx]
                # Combine inequalities and equalities into single constraint matrix
                A = torch.cat([A_ub, A_eq], dim=0)
                b = torch.cat([b_ub, b_eq], dim=0)
            elif len(ip_pkgs[ip_idx]) == 3:
                # Old format: (A, b, c)
                (A, b, c) = ip_pkgs[ip_idx]
            else:
                raise ValueError(f"Unexpected data format with {len(ip_pkgs[ip_idx])} elements")
            
            row, col, val = _dense_to_coo(A)
            ...
            for _ in range(self.rand_starts):
                ...
                if sol.intermediate is None or len(sol.intermediate) == 0:
                    print(f"Warning: IPM solver did not return intermediate solutions for instance {ip_idx}. Skipping.")
                    continue
                x = np.stack(sol.intermediate, axis=1)     # sol.intermediate are the intermediate values of x in inters
                if np.isnan(sol['fun']):
                    print(f"Warning: Solver returned NaN objective for instance {ip_idx}. Skipping.")
                    continue
        ...
        if not data_list:
            raise ValueError(f"No valid data instances were processed from {target_file}. Check the file content and solver results.")
        
        print(f"Successfully processed {len(data_list)} instances.")
...
class LPDataset_harp(InMemoryDataset):
    ...
    def process(self):
        # Find all numbered instance files
        import re
        numbered_files = []
        for f in os.listdir(self.raw_dir):
            if f.endswith('pkl.gz') and f.startswith('instance_'):
                match = re.match(r'instance_(\d+)\.pkl\.gz', f)
                if match:
                    numbered_files.append(int(match.group(1)))
        
        numbered_files.sort()
        num_instance_pkg = len(numbered_files)
        
        data_list = []
        for idx, i in enumerate(numbered_files):
            # load instance
            print(f"processing instance_{i}.pkl.gz ({idx+1}/{num_instance_pkg})")
            ...
            for ip_idx in tqdm(range(len(ip_pkgs))):
                ...
                row1, col1, val1 = _dense_to_coo(A1)
                row2, col2, val2 = _dense_to_coo(A2)
